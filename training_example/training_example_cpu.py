"""
PyTorch Integration Example: Glycan-Protein Binding Prediction (CPU Version)
CPU-based data loading to avoid CUDA OOM issues
Uses identical class names and method names as the original GPU version
"""
import torch
...
class PyTorchBindingPredictor:
    """
    PyTorch-based binding strength predictor using CPU-based DataLoaders
    Uses same class name as GPU version but CPU-based implementation to avoid OOM
    """
    ...
    def train(self,
        ...
        """
        Train the model using CPU-based PyTorch DataLoaders
...
def run_pytorch_pipeline():
    """Run complete PyTorch-based pipeline with CPU-based DataLoaders"""
    print("Running PyTorch Glycan-Protein Binding Pipeline (CPU Version)")
    print("CPU-based DataLoaders to avoid CUDA OOM")
    print("=" * 60)
    ...
    # CPU-safe settings
    batch_size = 32  # Can use larger batches since no GPU memory accumulation
    embedding_batch_size = 16  # Conservative for embedding computation
    ...
    try:
        ...
        # Create CPU-based DataLoaders
        logger.info("Creating CPU-based DataLoaders...")
        ...
        try:
            ...
            fig.suptitle('CPU-based Training Results', fontsize=16)
            ...
            plt.tight_layout()
            plt.savefig("cpu_pytorch_pipeline_results.png", dpi=300, bbox_inches='tight')
            plt.show()

        except ImportError:
            logger.info("Matplotlib not available, skipping plots")

        print(f"\nCPU-based PyTorch pipeline completed successfully!")
...
def test_cpu_dataloader():
    """Test CPU-based DataLoader functionality"""
    print("Testing CPU-based DataLoader")
    print("=" * 30)
