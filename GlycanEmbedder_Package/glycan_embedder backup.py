...
class GlycanGIN(BaseGlycanEmbedder):
    ...
    def __init__(self, input_dim, hidden_dims, num_unit, edge_input_dim=None, num_mlp_layer=2,
...
class GlycanMPNN(BaseGlycanEmbedder):
    ...
    def __init__(self, input_dim, hidden_dim, num_unit, edge_input_dim=None, num_layer=3,
        ...
        for i in range(num_mlp_layer):
            ...
            if i < num_mlp_layer - 1:
                if isinstance(activation, str):
                    message_layers.append(getattr(nn, activation.upper())())
                else:
...
class GlycanResNet(BaseGlycanEmbedder):
    ...
    def __init__(self, input_dim, hidden_dims, glycoword_dim, num_blocks=3, kernel_size=3,
...
class GlycanLSTM(BaseGlycanEmbedder):
    ...
    def __init__(self, input_dim, hidden_dim, glycoword_dim, num_layers=2, activation='tanh',
    ...
    def forward(self, graph, input=None, all_loss=None, metric=None):
        ...
        if self.bidirectional:
            ...
            hidden = hidden.mean(dim=1)  # (num_layers, batch, hidden_dim)

        # Reweight layers
        hidden = hidden.permute(1, 2, 0)  # (batch, hidden_dim, num_layers)
        graph_feature = self.reweight(hidden).squeeze(-1)  # (batch, hidden_dim)
...
if __name__ == "__main__":
    ...
    for method in ['gcn', 'lstm', 'cnn', 'bert']:
        embeds = embed_glycans(glycans, method=method, embedding_dim=64)
